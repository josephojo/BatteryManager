...
%         DEFAULT_BAUDRATE 		= uint32(499999); 	% Transfer Speed in bits per second (bps)
        DEFAULT_BAUDRATE 		= uint32(1000000); 	% Transfer Speed in bits per second (bps)
        ...
        DEFAULT_CSPIN 			= uint32(0x20);     % What pin is being used as CS. Default is GP5 or B00100000.
        
        DEFAULT_GPIO_PIN_DES    = uint8([0 1 2 2 2 1 2 0 0]); % LSB ... MSB. 0=GPIO, 1=CS, 2=DedicatdFunc
...
%         DEFAULT_GPIO_DIRECTION  = uint32(0x1DF);  % 0=Output, 1=Input
    ...
    methods (Access = private)
        ...
        function obj = vals2ptrs(obj)
            ...
            global spiModePtr activeCSstatePtr idleCSstatePtr baudRatePtr...
                cs2dataDlyPtr data2csDlyPtr data2dataDlyPtr txferSizePtr retPinValsPtr
    ...
    methods
        ...
        function [status, rxData] = receive(obj, rxSize)
            [status, rxData] = transfer(obj, uint8(0), rxSize);
        end
        ...
        function [status, spiExtReqStat, spiOwner] = cancelSpiXfer(obj)
            %cancelSpiXfer Cancel the current SPI transfer and provides information about:
            ...
            %   Outputs:
            %   	- uint8Ptr pspiExtReqStat           - SPI Bus release external request status:
            %       										- 0x01 – No External Request for SPI Bus Release
            %   											- 0x00 – Pending External Request for SPI Bus Release
            %   											Cannot be NULL.
            %   	- uint8Ptr pspiOwner                - SPI Bus current owner:
            %   											- 0x00 - No owner
            %   											- 0x01 - USB Bridge
            ...
            status = calllib(obj.libname,'Mcp2210_CancelSpiTxfer', ...
                obj.devHandle, spiExtReqStatPtr, spiOwnerPtr);
            obj.catchErr(status);
        ...
        function [status, pinVals] = getGPIOPinVals(obj)
            ...
            %	all pins configured as GPIO pins. 
        ...
        function [status, retPinVals] = setGPIOPinVals(obj, pinVals)
            %getGPIOPinVals Sets the states (Hi/Lo) for all GPIO pins
            %	The function gets the current GPIO Pin states/values of
            %	all pins configured as GPIO pins. 
            %   
            %   pinVals = a uint32 value corresponding to the bit
            %   equivalent of all 9 pins
            global retPinValsPtr
        ...
        function [status, pinVal] = readPin(obj, pinNum)
            %readPin Reads the digital value of a specified pin
            %   Input:
            ...
            %       - pinVal - 0 or 1 for off or on respectively
        ...
        function status = writeOFF2ON_us (obj, pinNum, usPeriod)
           %writeOFF2ON Writes a off and on to a specified pin for a period of time
           %    Input:
           %       - pinNum - Pin index on MCP2210 from 0 to 8
           %    
           %       - period - Time value in microseconds.
           
           prevTimer = toc;
            [~, currentPinVals] = getGPIOPinVals(obj);
            pinVals = bitset(currentPinVals, pinNum+1, 0,'uint32'); % pinNum+1 because matlab indexes from 1 not 0
            status1 = setGPIOPinVals(obj, pinVals);
            
            while (toc - prevTimer)*1E6 < usPeriod 
            end
            
            pinVals = bitset(currentPinVals, pinNum+1, 1,'uint32'); % pinNum+1 because matlab indexes from 1 not 0
            status2 = setGPIOPinVals(obj, pinVals);
            if status1 == 0 && status2 == 0
                status = 0;
            end
        end
        
        function status = writeOFF2ON_ns (obj, pinNum, nsPeriod)
           %writeOFF2ON Writes a off and on to a specified pin for a period of time
           %    Input:
           %       - pinNum - Pin index on MCP2210 from 0 to 8
           %    
           %       - period - Time value in NANOseconds.
           ...
           prevTimer = toc;
            ...
            while (toc - prevTimer)*1E9 < nsPeriod 
            end
