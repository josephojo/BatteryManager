classdef LTC6804
    ...
    methods
        ...
        % Wakeup function incase the devices are in sleep/idle mode
        function wakeup(obj)
            persistent wakeup_timestamp;
            if isempty(wakeup_timestamp)
                wakeup_timestamp=0;
            end
            ...
            if((wakeup_timestamp_new - wakeup_timestamp) < LTC6804.TSLEEP)
                ...
                LTC6804.DELAY_US(LTC6804.TREADY);disp("Ready");
            else
                % If wakeup signal sent more than LTC6804_TSLEEP time ago, then a long delay of LTC6804_TWAKE is required.
                LTC6804.DELAY_US(LTC6804.TWAKE);disp("Wake");
            end
            obj.spi.send(uint8(0));
            obj.spi.writePin(LTC6804.CS_PIN, 1);
