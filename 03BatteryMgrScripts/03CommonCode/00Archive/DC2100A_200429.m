        ...
        function USBDataIn_Callback(obj, s, ~)
            ...
            if strcmpi(s.BytesAvailableFcnMode, "byte")
                if s.NumBytesAvailable > 0
                    USB_In = read(s, 1, "char");
                    
%                     for ch = 1:length(USB_In)
                    obj.buf_in.add(USB_In(ch));
%                     end
                    
                    % If we've just received another command response, parse the
                    % previous response
                    key = obj.buf_in.peekLast();
        ...
        function USBDataIn_Parser(obj)
            ...
            %to workable values
            if obj.buf_in.size > 1 % Greater than one since we're assuming the next command is already stored
...
%                 disp(DC2100A.REMOVE_LEN(obj.buf_in, obj.buf_in.size));
                ...
                % If the element in the top of list is a command, goahead
                % and parse its data
                if(isKey(obj.USB_Parser_Response_DataSize, key))
                    response_length = obj.USB_Parser_Response_DataSize(key);
                    
                    if obj.buf_in.size >= response_length
                        
                        [status, obj] = USB_Process_Response(obj, response_length);
...
%                         obj.count = toc;
                ...
                case DC2100A.USB_PARSER_CELL_PRESENT_COMMAND
                    try
                        ...
                        if board_num >= DC2100A.MAX_BOARDS
                            ...
                            if (num_cells <= LTC3300.NUM_CELLS) 
                ...
                case DC2100A.USB_PARSER_EEPROM_COMMAND
                    try
                        ...
                        if board_num < DC2100A.MAX_BOARDS
                            ...
                            elseif item_num == DC2100A.EEPROM_Item_Type.Current
                                obj.EEPROM_Data(board_num +1).Capacity(temp_num +1)...
                                    = temp_eeprom_data.Capacity(temp_num +1);
                            else
        ...
        function USBTimerStopped(obj, varargin)
           ...
           disp("Instant Period = " + num2str(obj.USBTimer.InstantPeriod) + "s");
           disp("Average Period = " + num2str(obj.USBTimer.AveragePeriod) + "s");
        end
        ...
        function obj = DC2100A(COMport, errLogApp, varargin)
            ...
            try
                ...
                elseif status == ErrorCode.NO_ERROR
                    ...
                    obj.USBTimer.TasksToExecute = 50;
                    ...
                    num_cells_connected = 8;
                    obj = ConfigConnectedCells(obj, DC2100A.NUCLEO_BOARD_NUM, num_cells_connected);
...
%         function str = doThing(obj)
%             str = LTC3300_Raw_Read(obj,DC2100A.NUCLEO_BOARD_NUM, LTC3300.Command.Read_Balance);
%         end
        
        function obj = Cell_Present_Write(obj, board_num, varargin)
            ...
            if nargin > 2
               if isnumeric(varargin{1}) && varargin{1} <= DC2100A.MAX_CELLS
                   obj = ConfigConnectedCells(obj, varargin{1});
                   return;
               end
            end
