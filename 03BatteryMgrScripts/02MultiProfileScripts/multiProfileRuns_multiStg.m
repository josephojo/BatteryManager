...
%     figs = {};
    ...
    Purpose    = "MultiStage Charging NN Training Data (w/ SurfTemp) for P01_Rev5. 1C Only";
...
%     end
    ...
    iterations = 1;
prev3 = toc;
saveIntvl = 3; % Interval to save data (hr)
...
%                 initialSOC = prevSOC;
                ...
                elseif (initialSOC < targetSOC)
                    ...
                    disp(newline + "Iter: " + num2str(iter) + ...
                        ...
                        num2str(prevSOC * 100)+ "%. To: " +...
                        num2str(targetSOC * 100) + "%. ") 
                    adjCurrV = [1, 0.56234 0.31623, 0.17783, 0.1] * adjCurr;
                    out = chargeToVolt(highVoltLimit, adjCurrV(1),  'cellIDs', cellIDs);
                    
                    if ~isempty(out.Time)
                        resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                        prevSeqTime = prevSeqTime + out.Time(end);
                        ah = ah + out.Data(end, 9:10);
                    end
                    
                    out = chargeToVolt(highVoltLimit, adjCurrV(2),  'cellIDs', cellIDs);
                    if ~isempty(out.Time)
                        resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                        prevSeqTime = prevSeqTime + out.Time(end);
                        ah = ah + out.Data(end, 9:10);
                    ...
                    out = chargeToVolt(highVoltLimit, adjCurrV(3),  'cellIDs', cellIDs);
                    if ~isempty(out.Time)
                        resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                        prevSeqTime = prevSeqTime + out.Time(end);
                        ah = ah + out.Data(end, 9:10);
                    ...
                    out = chargeToVolt(highVoltLimit, adjCurrV(4),  'cellIDs', cellIDs);
                    if ~isempty(out.Time)
                        resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                        prevSeqTime = prevSeqTime + out.Time(end);
                        ah = ah + out.Data(end, 9:10);
                    ...
                    out = chargeToVolt(highVoltLimit, adjCurrV(5),  'cellIDs', cellIDs);
                    if ~isempty(out.Time)
                        resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                        prevSeqTime = prevSeqTime + out.Time(end);
                        ah = ah + out.Data(end, 9:10);
                ...
                if abs(prevSOC) <= 0.02 && targetSOC == 1 && ah(1) > 2 && ah(2) > 2
                    load(dataLocation + "007BatteryParam.mat");
                    batteryParam.capacity(cellIDs) = ah;
                    ahCounts(end+1, :) = ah;
...
%}
    ...
    if isMem == true %strcmpi(mode, 'cc')
        ...
        if strcmpi (cellConfig, 'parallel')
            adjCurr = sum(batteryParam.ratedCapacity(cellIDs))*3; % X of rated Capacity
        else
            adjCurr = batteryParam.ratedCapacity(cellIDs(1))*1; % X of rated Capacity
        end
        ...
        mode = modes2Run(ind);
        cycleSocTargets = csvread(dataLocation + '015cycleSocTargets.csv');
%         cycleSocTargets = [1, 0.1; 0.8, 0.1; 0.6, 0.1]; % ; 0.5, 0.1];
        startProfileInd = 1; % 6;
        endProfileInd = length(cycleProfiles);
        for i = 1:length(cycleSocTargets(:, 1)) %% FOR different SOC
            ...
            for ii = startProfileInd:endProfileInd
                ...
                if(prevSOC < initialSOC)
                    tSOC = 1;
                    c_rate = round(1+ rand(1)*4); % Random Values between 2 and 6
                    if strcmpi (cellConfig, 'parallel')
                    ...
                    disp(newline + "Iter: " + num2str(iter) + ...
                        ...
                        "Adjusting Initial SOC (Cycle): Charging to Full." + newline +...
                        "Adjust Current = " + num2str(adjCurr) + " A")
                    out = chargeTo(tSOC, adjCurr, 'cellIDs', cellIDs);
                    if ~isempty(out.Time)                    
                        resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                        prevSeqTime = prevSeqTime + out.Time(end);
                    end
                    ...
                    % If the previously recorded SOC was close to empty,
                    % record the 
                    if abs(prevSOC) <= 0.02 && tSOC == 1
                        load(dataLocation + "007BatteryParam.mat");
...
% Run Profile and Discharge -----------------------------------------------
                ...
                c_rate = round(2+ rand(1)*2); % Random Values between 2 and 6
                if strcmpi (cellConfig, 'parallel')
                    adjCurr = sum(batteryParam.ratedCapacity(cellIDs))*c_rate; % X of rated Capacity
                else
                    adjCurr = batteryParam.ratedCapacity(cellIDs(1))*1; % X of rated Capacity
                ...
                disp(newline + "Iter: " + num2str(iter) + ...
                    " | Profile # " + num2str(ii) + newline +...
                    "Discharge After Profile : Discharging to empty. " + newline +...
                        "Adjust Current = " + num2str(disCurr) + " A")
                out = dischargeTo(0, disCurr, 'cellIDs', cellIDs);
                if ~isempty(out.Time)                    
                    resultCollection{end+1} = [out.Time + prevSeqTime, out.Data];
                    prevSeqTime = prevSeqTime + out.Time(end);
                end
                disp("Discharged After Cycle" + newline);
