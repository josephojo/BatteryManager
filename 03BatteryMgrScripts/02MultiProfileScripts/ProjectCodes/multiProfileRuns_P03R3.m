...
try
    ...
    cellIDs = ["AA5"]; % ID in Cell Part Number (e.g BAT11-FEP-AA1). Defined again in initializeVariables
...
%     AhCounter(cellIDs, 1);
    ...
    if strcmpi (cellConfig, 'parallel')
        ...
        adjCurr = mean(batteryParam.ratedCapacity(cellIDs))*1; % X of rated Capacity
    end
...
%     Filename   = "AA6_NNData_03L_CY";
    Filename   = "AA5_NNData_03L_CY";
%     Purpose    = "Round 1 - NN CY Data after round 6.2 degrd (650 dgrd " +... 
%                     " cycles for AA6).";
    Purpose    = "Round 4 - NN CY Data after round 1 degrd (85 dgrd " +... 
                    " cycles for AA5). 1200 secs cool down period between cycles"; % 
    
%     Purpose    = "Round 1 - NN CY Data. No cool down period between cycles"; % 
    
    % This is the column where in resultCollection.Data (without time) is stored.
    AhCapInd = [4];  
...
for iter = 1:iterations
    ...
    if isMem == true 
        ...
        ccProfile = [3, 3, 2, 2, 2.5]; % Training Profiles
...
%}
                    if ccProfile(ind) == 3 && ccProfile(ind+1) == 2 || ...
                            ccProfile(ind) == 2 && ccProfile(ind+1) == 2.5
                        waitTime = 600; % in seconds
                    else
                        waitTime = 300; % in seconds
                    end
                    disp(newline + "Waiting after running CC Profile " + num2str(ii) + "C")
...
%                     end
                    ...
                    eval(['clear(resultCollection_' num2str(saveInd) ');']);
