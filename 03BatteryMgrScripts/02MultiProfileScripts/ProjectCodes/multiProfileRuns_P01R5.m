...
%     AhCounter(cellIDs);
    ...
    Purpose    = "Stationary NN Training Data (w/ SurfTemp) for P01_Rev5";
...
%}
    ...
    if isMem == true %strcmpi(mode, 'cc')
        ...
        if strcmpi (cellConfig, 'parallel')
            adjCurr = sum(batteryParam.ratedCapacity(cellIDs))*2; % X of rated Capacity
        else
            adjCurr = batteryParam.ratedCapacity(cellIDs(1))*1; % X of rated Capacity
        end
...
% Run Profile and Discharge -----------------------------------------------
                ...
                if strcmpi (cellConfig, 'parallel')
                    disCurr = sum(batteryParam.ratedCapacity(cellIDs))*c_rate; % X of rated Capacity
                else
                    disCurr = batteryParam.ratedCapacity(cellIDs(1))*1; % X of rated Capacity
                end
